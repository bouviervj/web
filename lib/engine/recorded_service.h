	...
	class recorded_service : public service {
		...
		public:
			...
			recorded_service(const std::string& iURLToRecord) : service() {
				...
				web::service_loader::getInstance().load(iURLToRecord,this);
	...
	class module_session_service :  public recorded_service {
		...
		public: 
			...
			virtual void proceed(http_context* iContext){
				...
				//Here decode context
				std::string aHash;
				void* aSessionContext;

				bool createCookie = false;
				http_cookie* aCookie = iContext->getCookie("web_sessionid");
				if (!aCookie) {
					createCookie = true;
				} else {
				...
				if (createCookie) {
					...
					iContext->addCookie("web_sessionid", aHash , "", "/", 200, false, true);

					_sessions_contexts[aHash] = NULL;
					aCookie = iContext->getCookie("web_sessionid");
					_module.initSession();
					aSessionContext = _module.getContext();
		...
		protected:
			...
			std::map<std::string, void*, strless> _sessions_contexts;
